...
from py3dtiles.points.task import (
    las_reader,
    xyz_reader,
    node_process,
    pnts_writer,
...
def make_rotation_matrix(z1, z2):
    ...
    return rotation_matrix(angle_between_vectors(v0, v1), vector_product(v0, v1))
    ...
    def run(self):
        ...
        while True:
            ...
            if delta > 0.01 and self.verbosity >= 1:
                print(
                    "{} / {} : Delta time: {}".format(
                        os.getpid(), round(after, 2), round(delta, 3)
                    )
                )
...
class State:
    ...
    def add_tasks_to_process(self, node_name, task, point_count):
        if point_count <= 0:
            raise ValueError(
                "point_count should be strictly positive, currently",
                point_count,
            )
...
class _Convert:
    def __init__(
        ...
        (
            ...
            self.original_aabb,
            self.avg_min,
        ) = self.get_rotation_matrix(srs_in, transformer)
        self.root_aabb, self.root_scale, self.root_spacing = self.get_root_aabb(
            self.original_aabb, keep_dimension
        )
        octree_metadata = OctreeMetadata(
            aabb=self.root_aabb,
            spacing=self.root_spacing,
            scale=self.root_scale[0],
        )
    ...
    def get_infos(self, color_scale, srs_in, srs_out):
        ...
        if len(extensions) != 1:
            raise ValueError(
                "All files should have the same extension, currently there are",
                extensions,
            )
        extension = extensions.pop()

        init_reader_fn = (
            las_reader.init if extension in (".las", ".laz") else xyz_reader.init
        )
        return init_reader_fn(
            self.files, color_scale=color_scale, srs_in=srs_in, srs_out=srs_out
        )
    ...
    def get_root_aabb(self, original_aabb, keep_dimension=False):
        base_spacing = compute_spacing(original_aabb)

        if not keep_dimension:
            if base_spacing > 10:
    ...
    def convert(self):
        ...
        while not self.zmq_manager.are_all_processes_killed():
            ...
            if (
                self.zmq_manager.are_all_processes_idle()
                and not self.zmq_manager.killing_processes
            ):
            ...
            if at_least_one_job_ended:
                ...
                if self.graph:
                    percent = round(
                        100 * self.state.processed_points / self.infos["point_count"],
                        3,
                    )
                    print(
                        "{}, {}".format(time.time() - self.startup, percent),
        ...
        if self.verbose >= 1:
            print(
                "destroy",
                round(self.zmq_manager.time_waiting_an_idle_process, 2),
            )
    ...
    def send_points_to_process(self, now):
        potentials = sorted(
            ...
            [
                (k, v)
                for k, v in self.state.node_to_process.items()
                if k not in self.state.processing_nodes
            ],
                ...
                pickle.dumps(
                    {
                        ...
                        "offset_scale": (
                            ...
                            self.infos["color_scale"].get(file)
                            if self.infos["color_scale"] is not None
                            else None,
                        ),
                        "portion": portion,
                    }
    ...
    def write_tileset(self):
        ...
        pnts_writer.node_to_pnts(
            "".encode("ascii"), root_node, self.out_folder, self.rgb
        )
        ...
        tileset = {
            ...
            "geometricError": np.linalg.norm(self.root_aabb[1] - self.root_aabb[0])
            / self.root_scale[0],
            "root": root_tileset,
        }
    ...
    def print_debug(self, now):
        ...
        if self.verbose >= 1:
            ...
            percent = round(
                100 * self.state.processed_points / self.infos["point_count"], 2
            )
            time_left = (100 - percent) * now / (percent + 0.001)
            print(
                "\r{:>6} % in {} sec [est. time left: {} sec]".format(
...
def init_parser(subparser, str2bool):
    ...
    parser.add_argument(
        "--srs_in",
        help="Override input SRS (numeric part of the EPSG code)",
        type=str,
    )
    parser.add_argument(
        "--fraction",
    ...
    parser.add_argument(
        "--rgb", help="Export rgb attributes", type=str2bool, default=True
    )
    parser.add_argument(
        "--graph",
        help="Produce debug graphes (requires pygal)",
        type=str2bool,
        default=False,
    )
