...
class ZmqManager:
    ...
    def __init__(self, number_of_jobs: int, process_args: tuple):
        ...
        self.processes = [Worker(*process_args, self.uri) for _ in range(number_of_jobs)]  # type: ignore
...
class _Convert:
    ...
    def send_points_to_process(self, now):
        ...
        potentials = sorted(
            ...
            [
                (node, task)
                for node, task in self.state.node_to_process.items()
                if node not in self.state.processing_nodes
            ],  # task: [data...], point_count
